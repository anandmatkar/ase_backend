...
const db_sql = {
    ...
    "Q15": `INSERT INTO machine
          (customer_id,project_id,order_id,machine_type, serial,description,manager_id, nom_speed, act_speed, hour_count) 
          VALUES('{var1}','{var2}','{var3}', '{var4}', '{var5}', '{var6}', '{var7}', '{var8}','{var9}','{var10}') RETURNING *`,
...
GROUP BY p.id, p.order_id, p.project_type, p.description, p.start_date, p.end_date, p.created_at, p.is_completed, p.is_requested_for_approval, p.manager_id, c.id, c.customer_name, c.customer_contact, c.customer_account, c.email_address, c.phone_number, c.country, c.city, c.address, c.scope_of_work;`,
                                            ...
                                            SELECT JSON_AGG(
                                                json_build_object(
                                                    ...
                                                    'project_report_data', COALESCE((
                                                        ...
                                                        AND pr.tech_id = tm.tech_id
                                                        AND pr.deleted_at IS NULL
                                                    )::json, '[]'::json)
                                                )
                                            )
                                            FROM tech_machine tm
                                            JOIN technician t ON tm.tech_id = t.id
                                            WHERE tm.machine_id = m.id
                                            AND tm.deleted_at IS NULL
                                        )
                                    )
                                )
        ...
        SELECT
            ...
            COALESCE(
                ...
                (
                    SELECT JSON_AGG(json_build_object(
                        ...
                        'project_report_data', COALESCE(
                            (
                                ...
                                WHERE pr.machine_id = m.id
                                AND pr.tech_id = tm.tech_id
                                AND pr.project_id = tm.project_id
                                AND pr.deleted_at IS NULL
                            ), '[]'
                        )
                    ))
                    FROM machine m
                    INNER JOIN tech_machine tm ON m.id = tm.machine_id
                    WHERE tm.tech_id = (SELECT id FROM unique_technicians)
                    AND tm.project_id = (SELECT id FROM project WHERE project.id = '{var1}')
                    AND tm.deleted_at IS NULL
                    AND m.deleted_at IS NULL
                ) AS machine_data
    ...
    "Q46": `UPDATE timesheet SET is_timesheet_approved = '{var1}', is_timesheet_requested_for_approval = '{var2}' WHERE project_id = '{var3}' AND tech_id = '{var4}' AND manager_id = '{var5}' AND deleted_at IS NULL RETURNING *`,
    "Q47": `SELECT
                        p.id AS project_id,
                        p.order_id,
                        p.customer_id,
    ...
    "Q83": `INSERT INTO signed_paper(project_id, tech_id, manager_id, file_path, file_type, file_size, file_name) VALUES('{var1}', '{var2}', '{var3}', '{var4}', '{var5}','{var6}', '{var7}') RETURNING *`,
    "Q84": `SELECT * FROM signed_paper WHERE project_id = '{var1}' AND tech_id = '{var2}' AND manager_id = '{var3}' AND deleted_at IS NULL`,
    "Q85": `UPDATE signed_paper SET deleted_at = '{var1}' WHERE tech_id = '{var2}' AND project_id = '{var3}' AND id = '{var4}' AND deleted_at IS NULL RETURNING *`,
